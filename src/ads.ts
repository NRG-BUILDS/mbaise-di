export const adverts = [
  {
    image:
      "https://images.unsplash.com/photo-1556742043-908ecf5f6d87?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80",
    title: "Best Dentistry in Lagos, Now 20% off",
    content:
      "Lorem ipsum dolor sit amet consectetur adipiscing elit Ut et massa mi. Aliquam in hendrerit urna. Pellentesque sit amet sapien fringilla, mattis ligula consectetur, ultrices mauris",
    contact: "emmanuelomolaki@gmail.com",
    client: "Emmanuel Dentist Clinic",
    link: "https://google.com/emmanuel-omolaju",
    action: "Learn more",
  },
  {
    image:
      "https://images.unsplash.com/photo-1624996379697-f01d168b1a52?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80",
    title: "Master Forex Trading in 30 Days",
    content:
      "Join our comprehensive forex trading class and learn the secrets of successful trading. From beginner to pro, we've got you covered.",
    contact: "forexmasterclass@gmail.com",
    client: "Forex Master Academy",
    link: "https://google.com/forex-master-class",
    action: "Enroll now",
  },
  {
    image:
      "https://images.unsplash.com/photo-1571260899304-425eee4c7efc?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80",
    title: "Apply for Admission at Greenfield Academy",
    content:
      "Greenfield Academy is now accepting applications for the new academic session. Secure your child's future with quality education.",
    contact: "admissions@greenfieldacademy.com",
    client: "Greenfield Academy",
    link: "https://google.com/greenfield-academy",
    action: "Apply now",
  },
];
export const news = [
  {
    image:
      "https://images.unsplash.com/photo-1593642532973-d31b6557fa68?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80",
    title: "Tech Innovations in 2025",
    content:
      "Discover the groundbreaking technologies set to revolutionize industries in 2025. From AI advancements to renewable energy solutions, the future is here.",
    date: "01/01/2025",
    photo_caption: "Tech World",
  },
  {
    image:
      "https://images.unsplash.com/photo-1517816743773-6e0fd518b4a6?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80",
    title: "Global Climate Summit 2025",
    content:
      "World leaders gather to discuss actionable steps to combat climate change. Key agreements and initiatives are expected to shape the planet's future.",
    date: "02/15/2025",
    photo_caption: "Climate Action",
  },
  {
    image:
      "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80",
    title: "Mars Mission: A Giant Leap for Humanity",
    content:
      "The first crewed mission to Mars is set to launch, marking a historic milestone in space exploration. Learn about the challenges and goals of this ambitious project.",
    date: "03/20/2025",
    photo_caption: "Space Exploration",
  },
];
